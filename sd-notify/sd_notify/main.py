...
# TODO: move to sd-client utils
# TODO: Get categories from sd-webui export (in the future from server key-val store)
...
sd = sd_client.ActivityWatchClient("sd-notify", testing=False)
...
def get_time(date=None) -> dict[str, timedelta]:
    ...
    hostname = sd.get_info().get("hostname", "unknown")
    canonicalQuery = sd_client.queries.canonicalEvents(
        sd_client.queries.DesktopQueryParams(
    ...
    res = sd.query(query, timeperiods)[0]
...
def main(verbose: bool):
    ...
    SD_CATEGORY_PATH = os.environ.get("SD_CATEGORY_PATH", None)
    if SD_CATEGORY_PATH:
        global CATEGORIES
        CATEGORIES = load_category_toml(Path(SD_CATEGORY_PATH))
        logger.info("Loaded categories from $SD_CATEGORY_PATH")
...
def get_active_status() -> Union[bool, None]:
    """
    Get active status by polling latest event in sd-watcher-afk bucket.
    Returns True if user is active/not-afk, False if not.
    On error, like out-of-date event, returns None.
    """

    hostname = sd.get_info().get("hostname", "unknown")
    events = sd.get_events(f"sd-watcher-afk_{hostname}", limit=1)
    logger.debug(events)
    if not events:
        return None
    event = events[0]
